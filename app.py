...
API_KEY = "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"  # Hugging Face API key
    ...
    <script>
        document.getElementById('classify-form').onsubmit = async function(event) {
            ...
            if (result.main_class && result.sub_class && result.position) {
                resultElement.innerText = `Main Class: ${result.main_class}, Subclass: ${result.sub_class}, Position: ${result.position}`;
                audioPlayer.src = `/audio/${result.sub_class}.wav`;
                audioPlayer.play();
                console.log(`Classified as: ${result.main_class}, Subclass: ${result.sub_class}, Position: ${result.position}`);
            } else {
...
async def classify():
    ...
    async with aiohttp.ClientSession() as session:
        try:
            ...
            result = {'main_class': main_class, 'sub_class': sub_class, 'position': position}
